...
struct KdGLTFMaterial
{
	...
	// 【2026/07/20 変更】Metallic の既定値を 1.0f → 0.0f にした(元の値はglTF仕様の既定に合わせたもの)。
	// マテリアルを1つも持たないモデル(Block.gltf=地面/Block/敵が使用)には、ローダーが
	// 既定マテリアルを1つ作って割り当てる(KdGLTFLoader.cpp「マテリアルがゼロの場合は、１つだけ作成しておく」)。
	// その既定が Metallic=1.0 だと「完全な金属」になり、シェーダ側で
	//   baseDiffuse  = lerp(baseColor, 黒, 1.0) = 0    ← 拡散光が完全に消える
	//   baseSpecular = lerp(0.04, baseColor, 1.0) = 白 ← 鏡面反射だけが残る
	// となる。その結果、地面の明るさが鏡面反射由来＝カメラ向き依存になり、
	// 「カメラを回すと地面全体の明るさが変わる」「影が見えない」という不具合が出ていた。
	// ※ マテリアルを持つモデルは KdGLTFLoader.cpp:245 で glTF の metallicFactor に
	//    上書きされるため、この既定値の影響を受けない(街の建物・キャラは変化なし)
	float			Metallic = 0.0f;			// 上記テクスチャのメタリック要素の乗算用　テクスチャが無い時は乗算ではなくそのまま使用する
