...
float4 main(VSOutput In) : SV_Target0
{
	...
	if (abs(liPos.x) <= 1 && abs(liPos.y) <= 1 && liPos.z <= 1)
	{
		...
		// ライトカメラからの距離(シャドウアクネ対策のバイアス付き)
		// 【2026/07/20 変更】固定値 0.004 → 面の傾きに連動させた(slope-scaled bias)。
		//
		// 元の固定バイアスには2つの問題があった:
		//  ・正規化された深度に対する値なので、実際のワールド距離は
		//    「0.004 x farZ」= 「0.006 x 影エリア」となり、影エリアを変えると実効値が勝手に変わる。
		//    影エリア100では0.6mで足りず、木組みの壁が自分自身に影を落として
		//    「家の骨組みが浮き出て見える」不具合が出ていた
		//  ・かといって固定値を大きくすると、光を正面から受ける面まで過剰にずれて
		//    影が物体から離れて浮く(ピーターパン現象)。影エリア200では1.2mになり、
		//    身長1.95mのキャラの影が体の2/3ぶんずれる計算になる
		//
		// アクネは「光に対して面が寝ているほど」出やすいので、その度合いだけ増やす。
		//  ・光を正面から受ける面(ndotl≈1) → バイアス最小 = 影が浮かない
		//  ・光に寝ている面(ndotl≈0)       → バイアス最大 = アクネが消える
		// これで両立するので、影エリアを下げて精細にする選択肢も戻る
		float ndotl = saturate(dot(wN, -g_DL_Dir));
		float shadowBias = 0.004 + 0.02 * (1.0 - ndotl);
		float z = liPos.z - shadowBias;
