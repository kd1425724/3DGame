...
float4 main(VSOutput In) : SV_Target0
{
	...
	// [SHADOWDEBUG] 影の値をそのまま白黒で出力する一時的な可視化(2026/07/20)。
	// 白=光が当たっている(shadow=1) / 黒=影の中(shadow=0)。
	// 「世界に固定された境界から先が暗い」の正体を、推測でなく直接見て切り分ける。
	//   境界の先が真っ黒 → その領域が影と誤判定されている(投影・深度範囲の問題)
	//   境界の先が真っ白 → shadowは正常。原因は拡散光側(法線・光の向き・マテリアル)
	//   境界が見えない   → shadowは無関係
	// ※ 確認後に必ずこのブロックごと削除する(grep: SHADOWDEBUG)
	//    定数バッファを増やすとC++側の配線が要るので、一時確認として無条件で返す
