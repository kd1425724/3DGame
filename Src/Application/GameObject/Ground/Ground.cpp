...
void Ground::Init()
{
	...
	// ※ 小規模な街(メインストリート型)に合わせたサイズ。街の範囲は X±20 / Z-43〜+55 なので、
	//    余白を持たせて X:±28(56) / Z:±65(130) にしてある。天面は y=+0.6(=1.2/2)で、
	//    Level.jsonの建物はこの高さに乗るよう pos.y を個別に計算して配置している。
	//    ステージを広げる時はここを大きくする(合わせてカリング距離/影/フォグの調整も要検討)
	SetScale(Math::Vector3(56.0f, 1.2f, 130.0f));
