...
public:
	...
	// カメラのロックオン注視に使う「体の安定した1点」(ワールド)。骨が無ければfalse。
	//
	// 【なぜ関節と分けるのか】狙える5関節のうち4つは四肢で、振り下ろし中の手は
	//   1フレームに9.48m動く(当たり判定を掃引にしたのと同じ理由)。カメラにそれを
	//   追わせると振り回されて酔う。ダークソウル等と同じく【カメラの対象】と
	//   【攻撃の対象】を分け、カメラは胴を見る。狙っている部位はレティクルが示す。
	// ※ 骨名は推測せずglTFのskins[0].jointsを読んで確認したもの(2026/08/08。全25本)
	bool GetCameraLockPos(Math::Vector3& _outPos) const;
...
private:
	...
	// 被弾リアクションの残り秒数。0より大きい間は "hit" を流し、移動を止める
	float m_hitReactTimer = 0.0f;

	// 被弾したので怯ませる。ダメージを受けた側から呼ぶ。
	// 【怯まない条件】倒れている間／攻撃中(ハイパーアーマー)
