...
public:
	...
	//  _thrustUp    ... 上下の噴射(+1で上/-1で下)。立体機動のガス噴射にあたる。
	//                   ワイヤーの引きに逆らって軌道を作るための、プレイヤー側の推進力
	// ※ たぐり寄せ(リール)は 2026/07/19 に一旦廃止。前進入力がワイヤーを縮めてしまい
	//    「前に行きたいのにアンカー側へ寄る」原因になっていたため。あとで別入力に割り当て直す予定
	void UpdateSwing(CharaBase& _body, float _dt, const Math::Vector2& _moveInput, float _thrustUp);
