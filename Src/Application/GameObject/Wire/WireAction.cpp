	...
	WireAction* const* _wires, int _count)
{
	...
	if (winchMode)
	{
		...
		else
		{
			for (int i = 0; i < n; ++i)
			{
				// 🔴 関節に刺さっているワイヤーは巻き取らない(2026/08/02)。
				// 巻き取りは「動かない支点へ手繰り寄せられて振られる」ための推進力だが、
				// 相手は歩き回る25mのゴーレムで、関節自体もアニメで大きく動く。
				// そこへ巻き取りを重ねると、支点の移動と手繰りが同じ向きに足し算されて
				// 制御できない勢いで引っ張られる(実機で確認)。
				// 敵に掛けたワイヤーは「距離の上限」としてだけ働かせ、
				// 動きはプレイヤー自身の入力とブーストに任せる
				if (live[i]->m_anchorJointIndex >= 0) { continue; }
