	...
	const Math::Vector3& _wallNormal)
{
	...
	// 火花として出す。テクスチャ(硬い芯のSpark.png)・色・大きさ・濃さ・寿命が
	// 噴射ジェットと別になる(調整値は「火花/〜」)。
	// 壁走りとワイヤーの着弾で共有する＝どちらも「金属が硬いものを擦る/噛む」同じ現象
	Add(std::make_shared<BoostEffect>(_pos + jitter, vel, BoostEffect::Style::Spark));
}
