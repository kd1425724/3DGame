...
- **pushは「セッションを終了する時にまとめて行う」（2026-08-03〜）。作業の区切りごとに「pushしますか」と聞かない。** 区切りの報告では`origin/main`より何コミット先行しているかを書くだけでよい
  - それ以前は「pushも都度確認」だったが、毎回聞かれるのが煩わしいとユーザーが指示して変更
- PR作成は引き続き別途確認する。`gh` CLIはこの環境に無いため、PRは手動作成用のURLを案内する
