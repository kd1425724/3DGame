...
**このファイルの運用**：コーディング規約・設計判断・ワークフローなど、今後のセッションにも影響する重要なルールが会話中に決まった場合は、**確認を取らずその場でこのファイルへ反映する**。改善点や新しいルールに気づいた場合も同様に、積極的に更新してよい。ただし**何をなぜ変えたかは必ず報告する**（黙って書き換えない）。
- **【2026-07-28より前の方針】**「追加してよいか確認すること」だったが、ユーザーの指示で撤回した。
...
7. **マジックナンバー・マジックストリングを直接書かない。** 名前付き定数（`constexpr`／`static constexpr`）にする。
   - ただし**調整対象の数値は定数ではなく`DebugParams`経由**にする（→DebugParamsの節）。「定数化」か「DebugParams化」のどちらかであって、リテラル直書きのまま残さない、という意味
   - 入力名の文字列を`main.cpp`へ集約する既存ルール（→入力の節）も同じ趣旨
8. **深いネストは早期リターンか関数分割でほどく。**
   - ガード節（`if (!x) { return; }`）を先に並べ、本体のネストを浅く保つ
   - `if`の入れ子が3段を超えたら関数分割を検討する
9. **コメントは「なぜ」を書き、「何を」は書かない。**
   - 不要（冗長）：`// カウンタを1増やす`のような、コードを読めば分かること
   - 必要（歓迎）：なぜその値・その順序・その回避策なのか。例：`// ランタイムがVersion17までしか読めないため`
   - **例外**：Framework内のコメントアウトした既存コードは、理由とセットで残す（→アーキテクチャの節）
...
- **値の「真実の源」は`DebugParams.json`（2026-07-21〜）。** `/Asset/Data/Debug/`を**gitで追跡**し、調整値をそのまま残す。コードの`Float()`第2引数は「JSONが無い環境でのフォールバック」という位置づけ。起動時に`Application::Init()`が`DebugParams::Instance().Load()`を呼び、JSONがあればそれが既定値を上書きする
- **運用フロー**：実行中に調整 → ImGuiのSaveボタンで保存 → **JSONをそのままコミットする**。**コードへの焼き込みは行わない**
- **注意**：日本語キーのためdiffが読みにくく、複数ブランチで同じ値を触るとマージ衝突しやすい。衝突したら「後から調整したほう」を採用してよい（調整値なので厳密な統合は不要）
- **【2026-07-21より前の方針】**「真実の源はコードの既定値。JSONはgitignoreして使い捨て、確定値はコードへ焼き込む」だったが、ユーザーの指示で撤回した。追跡しないとクリーンな環境や製品ビルドで調整値が反映されず（JSONが無いとコードの既定値へフォールバックする）、ファイルを失うと復元もできないため
- 対比：レベルデータ`Asset/Data/LevelEditor/*.json`も**ゲームの中身なので追跡する**。現在はどちらも追跡する方針
...
- **記録の置き場は auto-memory に一本化する。** 過去の決定・エラーと対処・プロジェクト固有の文脈は`C:\Users\kd142\.claude\projects\C--3DGame-BaseFramework-3D\memory\`に置く（毎セッション自動で読み込まれる）。**リポジトリに`MEMORY.md`/`ERRORS.md`は作らない** — 同じ情報が2箇所にあると必ず片方が古くなるため（引き継ぎ記録が16コミット分古くなっていた実例がある）
- 複雑なタスクは小さなステップに分解し、**各ステップの完了条件（何が観測できたら完了か）を先に決める**
- 大きなタスクの前に**複数のアプローチを提示して選ばせる**。1つに決め打ちしない
- **【検討の順番】①現実にその手法が実現できるか（世の中でどう作られているか）→ ②このフレームワークにあるか → ③無ければ追加を検討する、の順で見る。** フレームワークの制約から話を始めない。「エンジンに無い＝できない」ではなく、まず手法として成立するかを調べ、そのうえで実装手段を考える
- **【手法の紹介の書き方】選択肢は①②③…と番号で並べ、それぞれ「何をする手法か・どこで実際に使われているか・メリット/デメリット」を書く。** 図やASCIIの模式図を添えると伝わりやすい。どれが一番よく使われるかを明示し、最後に「このプロジェクトならどれか」で締める
- **分からないことは「分かりません」と言う。推測で答えない。** 観測できることは観測してから答える（推論で立て続けに外した実例がある）
- 症状ではなく**根本原因**を突き止める。「なぜそうなるか」を説明できるまで直したと言わない
- 変更したら**何を・なぜ変えたか・影響範囲**を簡潔に報告する
- プライバシーに配慮する。**画面キャプチャは毎回許可を取る**（こちらからは何が映っているか見えず、第三者の顔が写り込んだ事故がある）
