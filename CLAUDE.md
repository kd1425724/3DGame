...
6. **ブロックに文が2つ以上ある場合は1行に詰めず、複数行に展開する（各文を別々の行に）。文が1つだけのブロックは1行でよい。**
   - ダメ（2文以上を1行）：`{ float v = m_landingImpact; m_landingImpact = 0.0f; return v; }`
   - OK（1文）：`{ return; }`、`if (x < 0.0f) { x = 0.0f; }`
